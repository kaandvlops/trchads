...
// Types dosyasından artık Character'i çekiyoruz
// import { Character } from "@/types"; 

// Eğer types.ts dosyanı henüz güncellemediysen, hatayı engellemek için geçici olarak any yapıyorum. 
// Güncellediğinde 'any'leri 'Character' ile değiştir.
interface CharacterArenaProps {
  isOpen: boolean;
  onClose: () => void;
  allGames: any[]; // allCharacters olarak da isimlendirebilirdik ama props ismini bozmadım ana sayfada sorun çıkmasın
}

export default function CharacterArena({ isOpen, onClose, allGames }: CharacterArenaProps) {
  ...
  // Turnuvayı Sıfırdan Başlatan Ana Fonksiyon (Hem açılışta hem de Tekrar Başlat butonunda çalışır)
  ...
  // Modal ilk açıldığında turnuvayı kur
  useEffect(() => {
    if (isOpen) {
      startTournament();
    }
    ...
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-10 bg-black/80 backdrop-blur-md">
        <div className="bg-[#050505] border dergi-border p-12 text-center max-w-lg shadow-2xl">
          ...
          <h2 className="text-xl font-light text-white mb-8">Karakter Arenası'nın açılabilmesi için arşive en az 4 karakter eklemelisin.</h2>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-10 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
      ...
      <div className="relative w-full max-w-[80rem] h-[85vh] min-h-[600px] bg-[#050505] border dergi-border flex flex-col shadow-2xl overflow-hidden">
        ...
        {/* Üst Kapatma ve Durum Çubuğu */}
        <div className="absolute top-0 left-0 w-full p-6 flex justify-between items-start z-50 pointer-events-none">
          <div className="flex flex-col gap-2">
            <span className="bg-black/60 backdrop-blur-md px-4 py-2 border border-white/10 dergi-kicker text-indigo-400 mb-0 shadow-lg">
              Looksmax Arenası
            </span>
            {!champion && (
        ...
        {/* ŞAMPİYON EKRANI - Dergi Kapağı (Editorial) Tasarımı */}
        {champion ? (
          <div className="flex-1 w-full bg-[#050505] flex items-center justify-center p-6 md:p-16 animate-in fade-in duration-700">
            <div className="max-w-5xl w-full flex flex-col md:flex-row items-center gap-10 md:gap-16">
              ...
              {/* Sol: Görsel */}
              <div className="w-full max-w-[280px] md:max-w-md aspect-[3/4] relative border border-white/10 shadow-[0_0_40px_rgba(255,255,255,0.05)] bg-black shrink-0 overflow-hidden">
                {/* object-top ekledik yüzleri görebilmek için */}
              ...
              {/* Sağ: Metin ve Aksiyonlar */}
              <div className="flex-1 flex flex-col items-center md:items-start text-center md:text-left">
                <span className="dergi-kicker text-indigo-400 mb-4">Arena Şampiyonu</span>
                ...
                <div className="flex flex-col md:flex-row items-center gap-6">
                  {/* Link `/oyunlar` yerine `/karakterler` oldu */}
                  <Link href={`/karakterler/${champion.id}`} className="dergi-btn border-white text-white hover:bg-white hover:text-black px-8 py-4">
                    Karakter Analizini Gör
                  </Link>
                  <button 
            ...
            <div className="flex-1 w-full flex flex-col md:flex-row relative">
              ...
              >
                ...
                <div className="absolute bottom-8 left-8 md:bottom-12 md:left-12 z-10 translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                  ...
                  <p className="dergi-kicker text-indigo-400 opacity-0 group-hover:opacity-100 transition-opacity duration-700 delay-100 mt-2">Daha İyi (Seç)</p>
                </div>
              </div>
