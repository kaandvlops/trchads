...
// METİN AYRIŞTIRICI (Regex Güncellendi)
const renderParsedContent = (text: string) => {
  const parts = [];
  let lastIndex = 0;
  let mediaCount = 0;
  
  // YENİ REGEX: Hem pinterest.com/pin/12345 (match 5) hem de pin.it/xxxx (match 6) destekleniyor.
  const combinedRegex = /(https?:\/\/(?:www\.)?tiktok\.com\/@[\w.-]+\/video\/(\d+)(?:\?[^\s]*)?)|(https?:\/\/\S+\.gif)|(https?:\/\/(?:www\.|[a-z]{2}\.)?pinterest\.com\/pin\/(\d+)\/?)|(https?:\/\/pin\.it\/([a-zA-Z0-9]+))/gi;
  ...
  while ((match = combinedRegex.exec(text)) !== null) {
    ...
    if (mediaCount < 1) {
      if (match[2]) {
        parts.push({ type: 'tiktok', videoId: match[2] });
        mediaCount++;
      } else if (match[3]) {
        parts.push({ type: 'gif', url: match[3] });
        mediaCount++;
      } else if (match[5]) {
        parts.push({ type: 'pinterest', pinId: match[5] });
        mediaCount++;
      } else if (match[6]) {
        // KISA LINK: URL'nin kendisini alıyoruz
        parts.push({ type: 'pinterest_short', url: match[0] });
        mediaCount++;
      }
    } else {
  ...
  return parts.map((part, idx) => {
    ...
    if (part.type === 'tiktok') {
      return <TikTokSafeFacade key={idx} videoId={part.videoId as string} />;
    }
    
    if (part.type === 'pinterest') {
      return <PinterestSafeEmbed key={idx} pinId={part.pinId as string} />;
    }
    
    // YENİ: KISA LİNK (pin.it) GÖRÜNÜMÜ - Şık bir kırmızı buton
    if (part.type === 'pinterest_short') {
      return (
        <div key={idx} className="my-6">
          <a href={part.url as string} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-3 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/30 px-6 py-4 rounded-2xl transition-all font-mono text-xs uppercase tracking-widest group shadow-lg w-fit">
    ...
    if (part.type === 'gif') {
      return (
        <div key={idx} className="my-6 w-full max-w-xl md:ml-[-10px]">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img 
            src={part.url as string} 
            alt="User GIF" 
            className="rounded-lg border border-white/10 shadow-xl object-contain bg-[#050505] w-full" 
            loading="lazy"
          />
        </div>
      );
...
export default function CommentItem({ 
  comment, author, currentUserId, isAdmin, onReport, onWarn, onDelete, 
  ...
  const handleReplyClick = async (e: React.FormEvent) => {
    ...
    const success = await onReplySubmit(replyText, comment.id);
      ...
      {isReplying && (
        <form onSubmit={handleReplyClick} className={`mt-6 p-4 border dergi-border bg-[#020202] ${isReply ? 'md:ml-[64px]' : 'md:ml-[88px]'}`}>
          ...
          <div className="flex justify-end gap-3">
            <button type="button" onClick={() => setIsReplying(false)} className="text-[10px] font-mono tracking-widest uppercase text-white/40 hover:text-white transition-colors">
              İptal
            </button>
            <button type="submit" disabled={isSubmittingReply || !replyText.trim()} className="dergi-btn py-2 px-6 disabled:opacity-20 text-[10px]">
