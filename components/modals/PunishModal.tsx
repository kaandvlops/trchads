...
interface PunishModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (reason: string, banDuration: string) => Promise<void> | void;
  targetName?: string;
}
...
export default function PunishModal({ isOpen, onClose, onSubmit, targetName }: PunishModalProps) {
  ...
  // Modal her açıldığında seçimleri varsayılana sıfırla (yanlış ceza vermeyi önler)
  useEffect(() => {
    if (isOpen) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await onSubmit(reason, banDuration);
      onClose();
    } catch (error) {
  ...
  return (
    <BaseModal isOpen={isOpen} onClose={onClose} glowColor="yellow">
      ...
      <select 
        ...
        className="w-full bg-white/5 border border-white/10 text-white p-3 rounded-xl mb-4 outline-none focus:border-yellow-500/50 appearance-none font-light disabled:opacity-50"
      >
      ...
      <label className="block text-[10px] font-mono text-white/40 uppercase tracking-widest mb-2 ml-2">
        Uzaklaştırma (Ban) Süresi:
      </label>
      <select 
        value={banDuration} 
        disabled={isSubmitting}
        onChange={(e) => setBanDuration(e.target.value)}
        className="w-full bg-white/5 border border-white/10 text-white p-3 rounded-xl mb-8 outline-none focus:border-red-500/50 appearance-none font-light disabled:opacity-50"
      >
      ...
      <div className="flex gap-3">
        ...
        >
          {isSubmitting ? "İşleniyor..." : "İşlemi Onayla"}
        </button>
        <button 
          onClick={onClose} 
          disabled={isSubmitting}
